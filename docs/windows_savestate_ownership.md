...
| Doc | Role |
|---|---|
| `restore_invariants.md` | Class A/B/C + post-restore checklist |
| `docs/rabiribi_static_crt.md` | Allocator surface + false ucrtbase attribution |
| `crash_histogram.md` | 100-run death shape (harness) |
| `rewind_state_table.md` | Future multi-state / undo design (mostly unbuilt) |
...
`_malloc` `0x36E6B2`, `_free` `0x369754`, `_realloc` `0x36E744`, `__calloc_impl` `0x37EBF7`, `_msize` `0x37805A`, `FUN_00bdbe12` `0x38BE12`.  
No `_expand`. No `HeapCreate`/`HeapDestroy`.

**Non-CRT landlords on the same process heap:**  
`FUN_0087bc20` / `FUN_0087c070` (custom alloc/free), `FUN_008dfef0` (teardown-shaped).

**Settled implication:** CRT-only private heap is **incomplete isolation**. Cross-free between CRT and `FUN_0087*` ⇒ wrong-heap `HeapFree` (silent corruption). Arming stays gated until prove-disjoint or detour both.
...
| **Guard / hooks** | IAT / gated EXE patches; dsound track; optional CRT detours (off) | Wrong target (ucrtbase IAT on static CRT); incomplete free set |
...
| **Resume after load** | Island runs | **0 frames to death** cluster (histogram): wild pointers on workers/peers |
...
4. **CRT-only private heap is incomplete** while `FUN_0087bc20`/`c070` also use the process heap.  
...
2. Do `FUN_0087*` and CRT exchange blocks? (tag experiment)  
